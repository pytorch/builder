...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    time conda build -c soumith --no-anaconda-upload --python 3.6.0 pytorch-$BUILD_VERSION
    time conda build -c soumith --no-anaconda-upload --python 3.6.1 pytorch-$BUILD_VERSION    
else
    ...
    time conda build -c soumith --no-anaconda-upload --python 3.6.0 pytorch-cuda80-$BUILD_VERSION
    time conda build -c soumith --no-anaconda-upload --python 3.6.1 pytorch-cuda80-$BUILD_VERSION    
...
anaconda -t $ANACONDA_TOKEN upload --user soumith $(conda build -c soumith --python 3.6.0 pytorch-$BUILD_VERSION --output)
anaconda -t $ANACONDA_TOKEN upload --user soumith $(conda build -c soumith --python 3.6.1 pytorch-$BUILD_VERSION --output)
if [[ "$OSTYPE" == "linux"* ]]; then
    ...
    anaconda -t $ANACONDA_TOKEN upload --user soumith $(conda build -c soumith --python 3.6.0 pytorch-cuda80-$BUILD_VERSION --output)
    anaconda -t $ANACONDA_TOKEN upload --user soumith $(conda build -c soumith --python 3.6.1 pytorch-cuda80-$BUILD_VERSION --output)
fi
