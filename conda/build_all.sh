...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    time conda build --no-anaconda-upload --python 3.6 pytorch-$BUILD_VERSION
else
    ...
    time conda build --no-anaconda-upload --python 2.7 pytorch-cuda80-$BUILD_VERSION
    time conda build --no-anaconda-upload --python 3.5 pytorch-cuda80-$BUILD_VERSION
    time conda build --no-anaconda-upload --python 3.6 pytorch-cuda80-$BUILD_VERSION
...
time conda build --no-anaconda-upload --python 3.6 torchvision-$VISION_BUILD_VERSION
...
anaconda -t $ANACONDA_TOKEN upload --user soumith $(conda build --python 3.6 pytorch-$BUILD_VERSION --output)
if [[ "$OSTYPE" == "linux"* ]]; then
    anaconda -t $ANACONDA_TOKEN upload --user soumith $(conda build --python 2.7 pytorch-cuda80-$BUILD_VERSION --output)
    anaconda -t $ANACONDA_TOKEN upload --user soumith $(conda build --python 3.5 pytorch-cuda80-$BUILD_VERSION --output)
    anaconda -t $ANACONDA_TOKEN upload --user soumith $(conda build --python 3.6 pytorch-cuda80-$BUILD_VERSION --output)
fi
...
anaconda -t $ANACONDA_TOKEN upload --user soumith $(conda build --python 3.6 torchvision-$VISION_BUILD_VERSION --output)
