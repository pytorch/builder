...
PLATFORMS = ["manylinux1_x86_64", "manylinux2014_aarch64", "win_amd64", "macosx_10_9_x86_64", "macosx_11_0_arm64"]
...
def generate_expected_builds(platform: str, package: str, release: str) -> list:
    ...
    for py_version in PYTHON_VERSIONS:
        ...
        if package == "torchtext" and (platform == "manylinux2014_aarch64" or py_version == "cp312"):
            continue
        ...
        if "macos" in platform:
            if package == "torch":
                ...
                platform_spec = f"macosx_10_13_x86_64"

        builds.append(f"{package}-{release}-pypi-staging/{package}-{release}-{py_spec}-{platform_spec}.whl")
...
def validate_file_metadata(build: str, package: str, version: str):
    ...
    for i, line in enumerate(open(f"{temp_dir}/{package}-{version}.dist-info/METADATA")):
        if line.startswith(PATTERN_V):
            print(f"{line}", end='')
            exttracted_version = line.removeprefix(PATTERN_V).strip()
            if version != exttracted_version:
                print(f"FAILURE VERSION DOES NOT MATCH expected {version} got {exttracted_version}")
...
def main():
    ...
    for package in PACKAGE_RELEASES:
        for platform in PLATFORMS:
            expected_builds[package] = expected_builds[package] + generate_expected_builds(platform, package, PACKAGE_RELEASES[package])
