...
1. Modifying [`install_cuda.sh`](common/install_cuda.sh) and build Conda Docker file:
    ...
    4. Run the `install_116` chunk of code on your devbox to make sure it works.
    5. Check [this link](https://arnon.dk/matching-sm-architectures-arch-and-gencode-for-various-nvidia-cards/) to see if you need to add/remove any architectures to the nvprune list.
    6. Go into your cuda-11.6 folder and make sure what you're pruning actually exists. Update versions as needed, especially the visual tools like `nsight-systems`.
    7. Add setup for our Docker `conda` scripts/Dockerfiles
    8. To test that your code works, from the root builder repo, run something similar to `export CUDA_VERSION=11.3 && ./conda/build_docker.sh` for the `conda` images. 
    9. Validate conda-builder docker hub [cuda11.6](https://hub.docker.com/r/pytorch/conda-builder/tags?page=1&name=cuda11.6) to see that images have been built and correctly tagged. These images are used in the next step to build Magma for linux.

## 3. Update Magma for Linux
1. Build Magma for Linux. Our Linux CUDA jobs use conda, so we need to build magma-cuda116 and push it to anaconda:
...
1. Add setup for our Docker `libtorch` and `manywheel`:
    1. For `libtorch`, the code changes are usually copy-paste. For `manywheel`, you should manually verify the versions of the shared libraries with the CUDA you downloaded before.
    2. Create a new repo to host manylinux-cuda images, for example, https://hub.docker.com/r/pytorch/manylinux-cuda115. Then, give bots write and read access to this repo. This step can be removed once the following [issue](https://github.com/pytorch/builder/issues/901) is addressed.
...
## 5. Modify code to install the new CUDA for Windows
...
## 6. Update MAGMA for Windows
Compile MAGMA with the new CUDA version.
1. Our windows jobs download MAGMA binaries from our S3 `ossci-windows` bucket, which means that those binaries need to exist.
...
## 7. Add the new CUDA version to OSS CI.
...
## 8. Add the new CUDA version to the nightly binaries matrix.
